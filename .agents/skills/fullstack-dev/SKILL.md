...
description: >
  ...
  scaffolding backend service, building todo/CRUD/real-time/chat apps,
  Express + React, Next.js API, Node.js/Python/Go backend, designing service
  layers, implementing error handling, managing config/auth, setting up API
  clients, implementing auth flows, handling file uploads, adding real-time
  features (SSE/WebSocket), hardening for production.
  DO NOT TRIGGER when: pure frontend UI work, pure CSS/styling, database
  schema only, simple scripts, bug fixes, single-file changes, adding a small
  feature to existing code.
  触发词：全栈、后端服务、REST API、前后端联调、鉴权、WebSocket、生产加固。
license: MIT
metadata:
  version: 1.1.0
  category: full-stack
