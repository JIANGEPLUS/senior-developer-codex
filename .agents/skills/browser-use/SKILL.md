...
description: >
  ...
  and data extraction via the browser-use CLI. TRIGGER when: the user needs to
  navigate websites, interact with web pages, fill forms, take screenshots, or
  extract information from web pages; automate login-protected browsing;
  expose local dev servers via tunnel; run multi-session or cloud browser
  workflows. DO NOT TRIGGER for: plain HTTP API calls that need no browser,
  static file fetching.
  触发词：浏览器自动化、网页操作、截图、填表单、抓取网页、爬取数据。
license: MIT
metadata:
  version: 2.0.0
  category: automation
